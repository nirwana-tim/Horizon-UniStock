@forelse($schedules as $schedule)
    <tr class="hover:bg-gray-50">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $schedule->programLevel?->label ?? '-' }}</td>
        ...
        <td class="px-6 py-4 whitespace-nowrap">
            @if($schedule->student_type)
                <x-badge type="primary">{{ $schedule->student_type_label ?? ucfirst($schedule->student_type) }}</x-badge>
            @else

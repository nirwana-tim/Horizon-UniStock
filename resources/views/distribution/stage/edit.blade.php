<x-app-layout>
    <div class="flex items-center justify-between mb-5">
        <x-page-header title="Edit Distribution Stage" />
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('distribution.stages.update', $stage) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="stage_order" name="stage_order" type="number" class="mt-1 block w-full" :value="old('stage_order', $stage->stage_order)" min="0" required />
                    <x-input-error :messages="$errors->get('stage_order')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="start_date" :value="__('Start Date')" />
                    <x-text-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" :value="old('start_date', $stage->start_date?->format('Y-m-d'))" />
                    <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                </div>
                <div>
                    <x-input-label for="end_date" :value="__('End Date')" />
                    <x-text-input id="end_date" name="end_date" type="date" class="mt-1 block w-full" :value="old('end_date', $stage->end_date?->format('Y-m-d'))" />
                    <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
                </div>
                <div class="md:col-span-2">
                    <x-input-label for="notes" :value="__('Notes')" />
                    <textarea id="notes" name="notes" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500" rows="3">{{ old('notes', $stage->notes) }}</textarea>

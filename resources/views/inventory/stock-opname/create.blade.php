<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Create New Stock Opname Batch') }}</h2>
            <a href="{{ route('inventory.stock-opname.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 transition ease-in-out duration-150">{{ __('â† Back') }}</a>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('inventory.stock-opname.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <x-input-label for="opname_date" :value="__('Opname Date')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="period" name="period" type="text" class="mt-1 block w-full" :value="old('period')" required placeholder="e.g.: 2025/2026" />

<x-app-layout>
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <x-page-header title="Edit Event Ukuran" />

            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                <form action="{{ route('distribution.size-events.update', $sizeEvent) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <x-input-label value="Judul / Nama Event *" />
                        <input type="text" name="title" required value="{{ old('title', $sizeEvent->title) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2"
                            ...
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">{{ old('description', $sizeEvent->description) }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <x-input-label value="Waktu Mulai *" />
                            <input type="datetime-local" name="start_date" required value="{{ old('start_date', $sizeEvent->start_date?->format('Y-m-d\TH:i')) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                            <x-input-error :messages="$errors->get('start_date')" class="mt-1" />
                        </div>
                        <div>
                            <x-input-label value="Batas Akhir / Deadline *" />
                            <input type="datetime-local" name="end_date" required value="{{ old('end_date', $sizeEvent->end_date?->format('Y-m-d\TH:i')) }}"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                        ...
                        <div>
                            ...
                            <select name="student_level" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                                ...
                                @foreach($studentLevels as $st)
                                    <option value="{{ $st->kode }}" {{ old('student_level', $sizeEvent->student_level) == $st->kode ? 'selected' : '' }}>{{ $st->deskripsi }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="faculty_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                                ...
                                @foreach($faculties as $faculty)
                                    <option value="{{ $faculty->id }}" {{ old('faculty_id', $sizeEvent->faculty_id) == $faculty->id ? 'selected' : '' }}>{{ $faculty->name }} ({{ $faculty->code }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="study_program_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                                ...
                                @foreach($studyPrograms as $program)
                                    <option value="{{ $program->id }}" {{ old('study_program_id', $sizeEvent->study_program_id) == $program->id ? 'selected' : '' }}>{{ $program->faculty->code }} - {{ $program->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="max_changes" required value="{{ old('max_changes', $sizeEvent->max_changes) }}"
                            min="0" max="255"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm">
                    ...
                    <label class="flex items-start gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer">
                        <input type="checkbox" name="allow_reedit" value="1" {{ old('allow_reedit', $sizeEvent->allow_reedit) ? 'checked' : '' }}

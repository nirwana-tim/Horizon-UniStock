<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Detail Harga Item') }}</h2>
            <div class="flex gap-2">
                <a href="{{ route('master.item-price.edit', $itemPrice) }}" class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 transition">
                    {{ __('Edit') }}
                </a>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Tanggal Efektif</h3>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-200 flex gap-2">
                        <form action="{{ route('master.item-price.destroy', $itemPrice) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')">
                            @csrf
                            @method('DELETE')

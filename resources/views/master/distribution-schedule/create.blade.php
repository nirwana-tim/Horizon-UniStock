        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('master.distribution-schedule.store') }}" method="POST">
                        @csrf
                            ...
                            <div>
                                ...
                                <select id="stage_id" name="stage_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($stages as $stage)
                                        <option value="{{ $stage->id }}" {{ old('stage_id') == $stage->id ? 'selected' : '' }}>{{ $stage->name }} ({{ $stage->period?->name ?? '-' }})</option>
                                    @endforeach
                                </select>
                                ...
                                <div class="mt-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
                                    @foreach($items as $item)
                                        <label class="flex items-center space-x-2 p-2 border rounded hover:bg-gray-50">
                                            <input type="checkbox" name="item_ids[]" value="{{ $item->id }}" {{ in_array($item->id, old('item_ids', [])) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">

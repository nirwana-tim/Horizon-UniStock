            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('master-data.item-department.update', $itemDepartment) }}" method="POST">
                        ...
                        @php $selectedIdsStr = implode(',', $selectedIds) @endphp
                                ...
                                @foreach($faculties as $faculty)
                                    <div class="border border-gray-200 rounded-md p-3">
                                        <div class="flex items-center mb-2">
                                            ...
                                            <label for="faculty-{{ $faculty->id }}" @click="loadStudyPrograms({{ $faculty->id }}, '{{ route('master-data.item-department.study-programs', $faculty) }}?selected_ids={{ $selectedIdsStr }}')" class="ml-2 text-sm font-semibold text-gray-700 cursor-pointer">{{ $faculty->name }}</label>
                                        </div>
